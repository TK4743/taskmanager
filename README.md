...
flowchart TD
    subgraph Client["🖥️ IT Vault Presentation Layer (React 19 + TypeScript + Tailwind CSS v4 + Motion)"]
        ...
        UI_Sandbox["⚡ Monaco IDE Proctored Coding Studio"]
    ...
    subgraph Gateway["⚡ IT Vault API Gateway & Core Application Server (Express.js + Node.js 20+)"]
        ...
        ReadinessAI["🧠 Placement Readiness 2.0 & AI Skill Gap Engine"]
        BotEngine["🤖 Telegram Interactive Poller & 8:00 AM Cron Broadcast"]
        PushEngine["🔔 Web Push VAPID Notification Engine"]
        ReportEngine["📊 OpenXML ExcelJS, PDF & CSV Generator"]
        EmailPool["📧 Multi-Node Brevo HTTPS Email Dispatch Pool"]
        CacheLayer["⚡ In-Memory RAM Directory Cache (<0.01ms Lookups)"]
        Telemetry["📡 Sentry Performance Monitoring & Error Telemetry"]
    end

    subgraph Storage["💾 Persistence, Storage & External Cloud Services"]
        DB[("🗄️ PostgreSQL Database (35 Relational Tables with RLS)")]
        Cloudinary["🖼️ Cloudinary CDN (Image Proofs & Webcam Snapshots)"]
        LeetCodeAPI["☁️ LeetCode GraphQL Endpoint"]
        GitHubAPI["☁️ GitHub REST API v3"]
        TelegramAPI["📱 Telegram Bot API Gateway"]
        BrevoAPI["✉️ Brevo Multi-Node Transactional Email APIs"]
    end
...
```mermaid
flowchart TD
    Supreme["👑 Supreme Admin (Global Institutional Scope)"] --> HOD["🏛️ Head of Department (Department-Wide Scope)"]
    HOD --> Advisor["👨‍🏫 Class Advisor / Faculty (Section Scope: e.g. III IT-A)"]
    HOD --> Staff["🧑‍🏫 Staff / Subject Faculty (Course / R&D Scope)"]
    Advisor --> Coord["🎖️ Student Coordinator (Peer Review Scope)"]
    Coord --> Student["🎓 Student / Candidate (Individual Scope)"]
    
    Industry["🏢 Industry Partner / Recruiter (Corporate Talent Scope)"] -.->|Evaluates & Recruits| Student
    Industry -.->|Collaborates R&D| HOD
```
...
sequenceDiagram
    ...
    participant Alerts as Telegram Bot & Web Push

    Advisor->>Server: POST /api/tasks (Create Task with Deadline & Rubric)
    Server->>DB: INSERT INTO tasks
    Server->>Alerts: Broadcast Task Alert to Class Chat & Web Push
...
sequenceDiagram
    autonumber
    participant Daemon as Background Sync Daemon
    participant LC as LeetCode GraphQL API
    participant GH as GitHub REST API v3
    participant DB as PostgreSQL Database
    participant Telegram as Telegram Bot

    Note over Daemon: 23:50 IST: Daily Evening Progress Sync
    Daemon->>DB: Query active student profiles
    
    loop For Each Enrolled Student
        Daemon->>LC: Fetch solved problem counts, difficulty breakdown, global rank
        Daemon->>GH: Fetch public push events & daily commit totals
        Daemon->>DB: INSERT INTO leetcode_daily_progress & github_daily_progress
        Daemon->>DB: UPDATE student_coding_profiles (Cached Snapshot)
    end
    
    Note over Daemon: 08:00 AM IST: Morning Quota & Streak Evaluator
    Daemon->>DB: Calculate unmet coding targets & active streaks
...
erDiagram
    ...
    DEPARTMENTS ||--o{ CLASSES : includes
    USERS ||--o{ STUDENT_CODING_PROFILES : tracks
    USERS ||--o{ ASSESSMENT_ATTEMPTS : takes
    CODING_ASSESSMENTS ||--o{ CODING_ASSESSMENT_QUESTIONS : contains
    CODING_ASSESSMENT_QUESTIONS ||--o{ CODING_ASSESSMENT_TEST_CASES : has
    USERS ||--o{ CODING_ASSESSMENT_ATTEMPTS : attempts
    COMPANY_PROFILES ||--o{ INDUSTRY_POSTINGS : publishes
```
